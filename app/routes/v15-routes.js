...
router.post('/remove-v6/02-do-you-want-to-continue-with-a-saved-filing', function(request, response) {

  var savedFiling = request.session.data['saved-filing']
  if (savedFiling == "yes"){
      response.redirect("/remove-v6/03-your-filings")
  } else {
      response.redirect("/remove-v6/06a-entity-sold-all-land")
  }
})
...
router.post('/remove-v6/06a-entity-sold-all-land', function(request, response) {

  var entitySoldLand = request.session.data['entity-sold-land']
  if (entitySoldLand == "yes"){
      response.redirect("/remove-v6/06b-is-the-overseas-entity-registered-as-the-owner")
  } else {
      response.redirect("/remove-v6/06c-you-cannot-apply-to-remove-this-overseas-entity")
  }
})
...
router.post('/remove-v6/06b-is-the-overseas-entity-registered-as-the-owner', function(request, response) {

  var ownerDisposed = request.session.data['owner-disposed']
  if (ownerDisposed == "no"){
      response.redirect("/remove-v6/04-do-any-beneficial-owners-or-managing-officers-have-their-personal-information-protected-at-companies-house")
  } else {
      response.redirect("/remove-v6/06c-you-cannot-apply-to-remove-this-overseas-entity")
  }
})
...
// 04 ******* PROTECTED FILTER *******
router.post('/remove-v6/04-do-any-beneficial-owners-or-managing-officers-have-their-personal-information-protected-at-companies-house', function(request, response) {

  var secureRegister = request.session.data['secure-register']
  if (secureRegister == "yes"){
      response.redirect("/remove-v6/05-youll-need-to-file-an-update-statement-using-the-paper-form")
  } else {
      response.redirect("/remove-v6/06-are-there-any-trusts-involved-in-this-overseas-entity")
  }
})
...
router.post('/remove-v6/06-are-there-any-trusts-involved-in-this-overseas-entity', function(request, response) {
...
if (trustInvolved === "yes"){
  ...
  response.redirect('/remove-v6/08b-before-you-start')
} else {
...
router.post('/remove-v6/14-do-you-need-to-make-any-changes-to-this-overseas-entity', function(request, response) {

  var makeChanges = request.session.data['make-changes']
  if (makeChanges == "yes"){
      response.redirect("/remove-v6/19-who-is-completing-this-update")
  } else {
      response.redirect("/remove-v6/15-has-the-overseas-entity-identified-any-registrable-beneficial-owners")
  }
})



// router.get('/remove-v6/19-who-is-completing-this-update', function(req, res) {
//   if (req.session.data['make-changes'] === 'no') {
//     res.render('remove-v6/15b-has-the-overseas-entity-identified-any-registrable-beneficial-owners')
//   } else {
//     res.render('remove-v6/19-who-is-completing-this-update')
//   }
// })
...
router.post('/remove-v6/18-review-the-information-in-this-update-statement', function(request, response) {

  var nochangeChange = request.session.data['need-to-change']
  if (nochangeChange == "yes"){
      response.redirect("/remove-v6/xx-payment")
  } else {
      response.redirect("/remove-v6/14-do-you-need-to-make-any-changes-to-this-overseas-entity")
  }
})
